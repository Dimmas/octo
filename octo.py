...
def push_db(responce, cmd, lh):
    def write_db(prop):
        ...
        if expansion == '.odt':
            prop['text'] = lh.exec_command(f"odt2txt \"{prop['rcludi']}\"", fn)
        if expansion == '.docx':
            prop['text'] = lh.exec_command(f"docx2txt \"{prop['rcludi']}\" -", fn)
        if expansion == '.doc':
            prop['text'] = lh.exec_command(f"antiword \"{prop['rcludi']}\"", fn)
